...
def time_harmonic_solve(s,f_hat,K,Nx,rho,eps,precomp=None):
    if precomp is None:
        A,M,Bl,Br = spectral_galerkin_matrices(Nx,deg=50)
    else:
...
def make_mfe_sol(rho,eps,Nt,T,Nx,K,f,deg,xx,return_sg=False):
    ...
    rhs = create_rhs(Nt,T,Nx,K,f,precomp = precomp_pr,deg=deg)
...
def create_rhs(Nt,T,Nx,K,f,precomp = None,deg=50):
    ...
    fvals = [project_legendre_from_values(f(x_g,tau*j),Nx,precomp=precomp,deg=deg,x=x_g) for j in range(Nt+1)]
