...
def f(x,t):
    ...
    a = 100
    b = 10
    return np.exp(-b*(t-t0)**2)*np.heaviside(-0.25+x,0)*np.heaviside(0.75-x,0)
...
for rhoind in range(Am_rho):
    for epsind in range(Am_eps):
        ...
        filename = 'data/z_K_no_reg_rho_'+str(rho)+'_eps_'+str(eps)+'.npy'
...
fig.savefig('plots/z_Ks_decay_no_reg.pdf')
