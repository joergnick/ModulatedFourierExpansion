...
class Conv_Operator():
    ...
    def apply_convol_no_symmetry(self,rhs,T,show_progress=False,method="BDF2",cutoff=10**(-8)):
        ## Step 1
        if not len(rhs[0]):
...
#######################     
        ...
        for j in range(1,L+1):
            ...
            if normsRHS[j]>cutoff:
                if show_progress:
                    #print("j:",j,"L:",str(L), "Time of previous iteration: " +str((end-start)/60), " MIN" )
                    print("j:",j,"L:",L, "Norm right-hand side: " +str(normsRHS[j]), " MIN" )
                start=time.time()
