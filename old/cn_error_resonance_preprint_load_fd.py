...
for rhoind in range(1):
    for epsind in range(1):
        rho = 0.4*2**(-rhoind)
        #rho = 0.4*2**(-rhoind)
        eps = np.round(0.04*0.1**(epsind),5)
      ...
      # --- Matlab-like styling ---
        ...
        # Main curves
        plt.loglog(taus, mfe_errs, '-o', label='MFE-TR')
        plt.loglog(taus, cn_errs, '-s', label='TR')
        
        # Reference slopes
        #plt.loglog(T / Nts, T / Nts, '--', label=r'$h$')
        plt.loglog(taus, taus**2, '--', label=r'$\tau^2$')
